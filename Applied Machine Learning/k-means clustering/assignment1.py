...
def analyze_kmeans():
    ...
    """
    X = np.genfromtxt("digit.txt") # (1000, 157)
    y = np.genfromtxt("labels.txt", dtype=int) # (1000, )
    distortions = []
    errs = []
    ks = range(1, 11)
    for k in ks:
...
def assign(X, Mu):
    ...
    for i in range(len(Mu)):
        z[:,i] = np.sum(np.square(X-Mu[i]),axis=1)
...
def update(X, z, k):
    ...
    for i in range(k):
        Mu[i, :] = np.sum(X[z == i, :], axis=0) / uniq_cnt_dict[i]
    return Mu
...
def compute_distortion(X, Mu, z):
    ...
    distortion = 0
    for i in range(len(Mu)):
        distortion += np.sum(np.square(X[z==i,:]-Mu[i]))
    return distortion
...
def label_clusters(y, k, z):
    ...
    labels = np.zeros(k)
    for i in range(k):
        unique, counts = np.unique(y[z == i], return_counts=True)
        uniq_cnt_dict = dict(zip(unique, counts))
        labels[i] = sorted(uniq_cnt_dict.items(), key=lambda x: x[1], reverse=True)[0][0]
    return labels
...
'''
question:
Write a few sentences with your thoughts on these results. Are they what
you expected? Did you think that within-group sum of squares and
mistake rate would go up or decrease as k increased? Did the plots
confirm your expectations?

answer:
The results on my code show expected appropriate results.
I think within-group sum of squares and mistake rate would decrease as k increased. 
However, if the value of k increases more, within-group sum of squares and mistake rate would have similar outcomes which are not enhanced anymore. 
The plots I made confirm my expectations.
'''